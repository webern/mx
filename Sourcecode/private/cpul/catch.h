...
namespace Catch
{
    ...
    #else
    template<typename Func, typename U> using FunctionReturnType = typename std::remove_reference<typename std::remove_cv<
...
namespace Catch
{
    ...
    public:
        ...
        {
            static_assert(
                always_false<T>::value, "chained comparisons are not supported inside assertions, "
    ...
    public:
        ...
        {
            static_assert(
                always_false<RhsT>::value, "operator&& is not supported inside assertions, "
                                           "wrap the expression inside parentheses, or decompose it"
            );
        }
...
namespace Catch
{
    ...
    struct IExceptionTranslator
    {
        ...
        virtual std::string translate(
            ExceptionTranslators::const_iterator it, ExceptionTranslators::const_iterator itEnd
        ) const = 0;
    };
...
namespace Catch
{
    namespace Generators
    {
        ...
        {
            static_assert(
                !std::is_same<T, bool>::value, "RepeatGenerator currently does not support bools"
...
//       but I don't expect users to run into that in practice.
        ...
        typename std::enable_if<std::is_integral<T>::value && !std::is_same<T, bool>::value,
                                GeneratorWrapper<T>>::type random( T a, T b )
        {
            return GeneratorWrapper<T>(
        ...
        {
            static_assert(
                !std::is_same<T, bool>::value, "IteratorGenerator currently does not support bools"
                                               "because of std::vector<bool> specialization"
            );
        ...
        template<typename InputIterator, typename InputSentinel, typename ResultType = typename std::iterator_traits<
