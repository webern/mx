...
{
  public:
    explicit impl() : myDecimal(0), myIsNormal(true)
    {
    }

    explicit impl(const Decimal &value) : myDecimal(value), myIsNormal(false)
    {
    }

    explicit impl(const std::string &value) : myDecimal(0), myIsNormal(false)
    {
        parse(value);
    }
...
NumberOrNormal::NumberOrNormal() : myImpl(new impl())
{
}

NumberOrNormal::NumberOrNormal(const Decimal &value) : myImpl(new impl(value))
{
}

NumberOrNormal::NumberOrNormal(const std::string &value) : myImpl(new impl(value))
{
}
...
NumberOrNormal::NumberOrNormal(const NumberOrNormal &other) : myImpl(new NumberOrNormal::impl(*other.myImpl))
{
}

NumberOrNormal::NumberOrNormal(NumberOrNormal &&other) : myImpl(std::move(other.myImpl))
{
}
