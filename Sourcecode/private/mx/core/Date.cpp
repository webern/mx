...
{
  public:
    impl(int year_in, int month_in, int day_in) : myYear(1900), myMonth(1), myDay(1)
    {
        setYear(year_in);
        setMonth(month_in);
        setDay(day_in);
    ...
    impl() : myYear(1900), myMonth(1), myDay(1)
    {
    }

    impl(const std::string &yyyy_mm_dd) : myYear(1900), myMonth(1), myDay(1)
    {
  ...
  public:
    ...
    bool parseString(const std::string &value_in)
    {
        if (isStringFormatValid(value_in))
        {
            ...
            int month = -1;
            int day = -1;
            if (parseStringYear(value_in, year) && parseStringMonth(value_in, month) && parseStringDay(value_in, day))
            {
...
Date::Date(int year_in, int month_in, int day_in) : myImpl(new impl(year_in, month_in, day_in))
{
}

Date::Date() : myImpl(new impl())
{
}

Date::Date(const std::string &yyyy_mm_dd) : myImpl(new impl(yyyy_mm_dd))
{
}
...
Date::Date(const Date &other)
    : myImpl(new impl(other.myImpl->getYear(), other.myImpl->getMonth(), other.myImpl->getDay()))
{
}

Date::Date(Date &&other) : myImpl(std::move(other.myImpl))
{
}
...
{
    this->myImpl =
        std::unique_ptr<impl>(new impl(other.myImpl->getYear(), other.myImpl->getMonth(), other.myImpl->getDay()));
    return *this;
}
