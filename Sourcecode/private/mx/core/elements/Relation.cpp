...
namespace core
{
Relation::Relation() : myValue(), myAttributes(std::make_shared<RelationAttributes>())
{
}

Relation::Relation(const XsString &value) : myValue(value), myAttributes(std::make_shared<RelationAttributes>())
{
}
...
bool Relation::fromXElementImpl(std::ostream &message, ::ezxml::XElement &xelement)
{
    bool isSuccess = true;
