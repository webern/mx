...
namespace core
{
BeaterAttributes::BeaterAttributes() : tip(), hasTip(false)
{
}
...
bool BeaterAttributes::fromXElementImpl(std::ostream &message, ::ezxml::XElement &xelement)
{
    ...
    for (; it != endIter; ++it)
    {
        if (parseAttribute(message, it, className, isSuccess, tip, hasTip, "tip", &parseTipDirection))
        {
            continue;
        }
    }
