...
{
Print::Print()
    : myAttributes(std::make_shared<PrintAttributes>()), myLayoutGroup(makeLayoutGroup()),
      myMeasureLayout(makeMeasureLayout()), myHasMeasureLayout(false), myMeasureNumbering(makeMeasureNumbering()),
      myHasMeasureNumbering(false), myPartNameDisplay(makePartNameDisplay()), myHasPartNameDisplay(false),
      myPartAbbreviationDisplay(makePartAbbreviationDisplay()), myHasPartAbbreviationDisplay(false)
{
}
...
{
    return myLayoutGroup->hasContents() || myHasMeasureLayout || myHasMeasureNumbering || myHasPartNameDisplay ||
...
bool Print::fromXElementImpl(std::ostream &message, ::ezxml::XElement &xelement)
{
    ...
    for (auto it = xelement.begin(); it != endIter; ++it)
    {
        ...
        if (importElement(message, *it, isSuccess, *myMeasureLayout, myHasMeasureLayout))
        {
            continue;
        }
        if (importElement(message, *it, isSuccess, *myMeasureNumbering, myHasMeasureNumbering))
        {
            continue;
        }
