...
{
Lyric::Lyric()
    : myAttributes(std::make_shared<LyricAttributes>()), myLyricTextChoice(makeLyricTextChoice()),
...
bool Lyric::fromXElementImpl(std::ostream &message, ::ezxml::XElement &xelement)
{
    ...
    for (auto it = xelement.begin(); it != endIter; ++it)
    {
        ...
        else if (checkSetChoiceMember(message, *it, isSuccess, myLyricTextChoice, "laughing",
        ...
        if (importElement(message, *it, isSuccess, *myEndLine, myHasEndLine))
        {
            continue;
        }
        if (importElement(message, *it, isSuccess, *myEndParagraph, myHasEndParagraph))
        {
            continue;
        }
