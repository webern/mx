...
{
Defaults::Defaults()
    : myScaling(makeScaling()), myHasScaling(false), myLayoutGroup(makeLayoutGroup()), myAppearance(makeAppearance()),
      myHasAppearance(false), myMusicFont(makeMusicFont()), myHasMusicFont(false), myWordFont(makeWordFont()),
      myHasWordFont(false), myLyricFontSet(), myLyricLanguageSet()
{
}
...
{
    return myHasScaling || myLayoutGroup->hasContents() || myHasAppearance || myHasMusicFont || myHasWordFont ||
           myLyricFontSet.size() > 0 || myLyricLanguageSet.size() > 0;
}
...
bool Defaults::fromXElementImpl(std::ostream &message, ::ezxml::XElement &xelement)
{
    ...
    {
        if (importElement(message, *it, isSuccess, *myScaling, myHasScaling))
        {
            continue;
        }
        importGroup(message, it, endIter, isSuccess, myLayoutGroup);
        if (importElement(message, *it, isSuccess, *myAppearance, myHasAppearance))
        {
            continue;
        }
        if (importElement(message, *it, isSuccess, *myMusicFont, myHasMusicFont))
        {
            continue;
        }
        if (importElement(message, *it, isSuccess, *myWordFont, myHasWordFont))
        {
            continue;
        }
