...
MeasureStyleAttributes::MeasureStyleAttributes()
    : number(), fontFamily(), fontStyle(FontStyle::normal), fontSize(CssFontSize::medium),
      fontWeight(FontWeight::normal), color(), hasNumber(false), hasFontFamily(false), hasFontStyle(false),
...
{
    return hasNumber || hasFontFamily || hasFontStyle || hasFontSize || hasFontWeight || hasColor;
}
...
bool MeasureStyleAttributes::fromXElementImpl(std::ostream &message, ::ezxml::XElement &xelement)
{
