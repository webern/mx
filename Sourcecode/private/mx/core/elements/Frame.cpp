...
{
Frame::Frame()
    : myAttributes(std::make_shared<FrameAttributes>()), myFrameStrings(makeFrameStrings()),
      myFrameFrets(makeFrameFrets()), myFirstFret(makeFirstFret()), myHasFirstFret(false), myFrameNoteSet()
{
...
bool Frame::fromXElementImpl(std::ostream &message, ::ezxml::XElement &xelement)
{
    bool isSuccess = true;
    ...
    {
        if (importElement(message, *it, isSuccess, *myFrameStrings, isFrameStringsFound))
        {
            continue;
        }
        if (importElement(message, *it, isSuccess, *myFrameFrets, isFrameFretsFound))
        {
            continue;
        }
        if (importElement(message, *it, isSuccess, *myFirstFret, myHasFirstFret))
        {
            continue;
        }
