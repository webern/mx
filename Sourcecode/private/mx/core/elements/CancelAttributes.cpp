...
namespace core
{
CancelAttributes::CancelAttributes() : location(), hasLocation(false)
{
}
...
bool CancelAttributes::fromXElementImpl(std::ostream &message, ::ezxml::XElement &xelement)
{
    ...
    for (; it != endIter; ++it)
    {
        if (parseAttribute(message, it, className, isSuccess, location, hasLocation, "location", &parseCancelLocation))
        {
            continue;
        }
    }
