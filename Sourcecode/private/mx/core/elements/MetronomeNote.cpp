...
MetronomeNote::MetronomeNote()
    : myMetronomeType(makeMetronomeType()), myMetronomeDotSet(), myMetronomeBeamSet(),
      myMetronomeTuplet(makeMetronomeTuplet()), myHasMetronomeTuplet(false)
{
}
...
bool MetronomeNote::fromXElementImpl(std::ostream &message, ::ezxml::XElement &xelement)
{
    bool isSuccess = true;
    ...
    {
        if (importElement(message, *it, isSuccess, *myMetronomeType, isMetronomeTypeFound))
        {
            continue;
        }
        ...
        if (importElement(message, *it, isSuccess, *myMetronomeTuplet, myHasMetronomeTuplet))
        {
            continue;
        }
    }
