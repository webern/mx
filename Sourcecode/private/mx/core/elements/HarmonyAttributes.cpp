...
HarmonyAttributes::HarmonyAttributes()
    : type(), printObject(YesNo::no), printFrame(YesNo::no), defaultX(), defaultY(), relativeX(), relativeY(),
      fontFamily(), fontStyle(FontStyle::normal), fontSize(CssFontSize::medium), fontWeight(FontWeight::normal),
      color(), placement(AboveBelow::below), hasType(false), hasPrintObject(false), hasPrintFrame(false),
...
{
    return hasType || hasPrintObject || hasPrintFrame || hasDefaultX || hasDefaultY || hasRelativeX || hasRelativeY ||
...
bool HarmonyAttributes::fromXElementImpl(std::ostream &message, ::ezxml::XElement &xelement)
{
    ...
    for (; it != endIter; ++it)
    {
        if (parseAttribute(message, it, className, isSuccess, type, hasType, "type", &parseHarmonyType))
        {
            continue;
        }
        ...
        if (parseAttribute(message, it, className, isSuccess, printFrame, hasPrintFrame, "print-frame", &parseYesNo))
        {
            continue;
        }
