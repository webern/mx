...
namespace core
{
MidiDeviceAttributes::MidiDeviceAttributes() : port(), id(), hasPort(false), hasId(false)
{
}

bool MidiDeviceAttributes::hasValues() const
{
    return hasPort || hasId;
}
...
bool MidiDeviceAttributes::fromXElementImpl(std::ostream &message, ::ezxml::XElement &xelement)
{
    ...
    for (; it != endIter; ++it)
    {
        if (parseAttribute(message, it, className, isSuccess, port, hasPort, "port"))
        {
            continue;
        }
        if (parseAttribute(message, it, className, isSuccess, id, hasId, "id"))
        {
            continue;
        }
    }
