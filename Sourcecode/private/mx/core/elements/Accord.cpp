...
{
Accord::Accord()
    : myAttributes(std::make_shared<AccordAttributes>()), myTuningStep(makeTuningStep()),
      myTuningAlter(makeTuningAlter()), myHasTuningAlter(false), myTuningOctave(makeTuningOctave())
{
}
...
bool Accord::fromXElementImpl(std::ostream &message, ::ezxml::XElement &xelement)
{
    bool isSuccess = true;
    ...
    {
        if (importElement(message, *it, isSuccess, *myTuningStep, isTuningStepFound))
        {
            continue;
        }
        if (importElement(message, *it, isSuccess, *myTuningAlter, myHasTuningAlter))
        {
            continue;
        }
        if (importElement(message, *it, isSuccess, *myTuningOctave, isTuningOctaveFound))
        {
            continue;
        }
    }
