...
{
    return hasDefaultX || hasDefaultY || hasRelativeX || hasRelativeY || hasColor || hasHalign || hasValign ||
           hasHeight || hasWidth || hasUnplayed;
}
...
bool FrameAttributes::fromXElementImpl(std::ostream &message, ::ezxml::XElement &xelement)
{
    ...
    for (; it != endIter; ++it)
    {
        ...
        if (parseAttribute(message, it, className, isSuccess, height, hasHeight, "height"))
        {
            continue;
        }
        ...
        if (parseAttribute(message, it, className, isSuccess, unplayed, hasUnplayed, "unplayed"))
        {
            continue;
        }
    }
