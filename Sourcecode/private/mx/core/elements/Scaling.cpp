...
namespace core
{
Scaling::Scaling() : myMillimeters(makeMillimeters(MillimetersValue(7))), myTenths(makeTenths(TenthsValue(40)))
{
}
...
bool Scaling::fromXElementImpl(std::ostream &message, ::ezxml::XElement &xelement)
{
    ...
    {
        if (importElement(message, *it, isSuccess, *myMillimeters, isMillimetersFound))
        {
            continue;
        }
        if (importElement(message, *it, isSuccess, *myTenths, isTenthsFound))
        {
            continue;
        }
    }
