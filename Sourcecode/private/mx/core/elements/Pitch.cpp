...
namespace core
{
Pitch::Pitch() : myStep(makeStep()), myAlter(makeAlter()), myHasAlter(false), myOctave(makeOctave())
{
}
...
bool Pitch::fromXElementImpl(std::ostream &message, ::ezxml::XElement &xelement)
{
    ...
    {
        if (importElement(message, *it, isSuccess, *myStep, isStepFound))
        {
            continue;
        }
        if (importElement(message, *it, isSuccess, *myAlter, myHasAlter))
        {
            continue;
        }
        if (importElement(message, *it, isSuccess, *myOctave, isOctaveFound))
        {
            continue;
        }
    }
