...
TechnicalChoice::TechnicalChoice()
    : myChoice(Choice::upBow), myUpBow(makeUpBow()), myDownBow(makeDownBow()), myHarmonic(makeHarmonic()),
      myOpenString(makeOpenString()), myThumbPosition(makeThumbPosition()), myFingering(makeFingering()),
      myPluck(makePluck()), myDoubleTongue(makeDoubleTongue()), myTripleTongue(makeTripleTongue()),
      myStopped(makeStopped()), mySnapPizzicato(makeSnapPizzicato()), myFret(makeFret()), myString(makeString()),
      myHammerOn(makeHammerOn()), myPullOff(makePullOff()), myBend(makeBend()), myTap(makeTap()), myHeel(makeHeel()),
      myToe(makeToe()), myFingernails(makeFingernails()), myHole(makeHole()), myArrow(makeArrow()),
      myHandbell(makeHandbell()), myOtherTechnical(makeOtherTechnical())
{
}
