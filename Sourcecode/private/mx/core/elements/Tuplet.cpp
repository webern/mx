...
{
Tuplet::Tuplet()
    : myAttributes(std::make_shared<TupletAttributes>()), myTupletActual(makeTupletActual()), myHasTupletActual(false),
      myTupletNormal(makeTupletNormal()), myHasTupletNormal(false)
{
}
...
bool Tuplet::fromXElementImpl(std::ostream &message, ::ezxml::XElement &xelement)
{
    ...
    {
        if (importElement(message, *it, isSuccess, *myTupletActual, myHasTupletActual))
        {
            continue;
        }
        if (importElement(message, *it, isSuccess, *myTupletNormal, myHasTupletNormal))
        {
            continue;
        }
    }
