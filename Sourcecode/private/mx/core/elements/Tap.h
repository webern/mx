...
inline TapPtr makeTap()
{
    return std::make_shared<Tap>();
}

inline TapPtr makeTap(const XsString &value)
{
    return std::make_shared<Tap>(value);
}

inline TapPtr makeTap(XsString &&value)
{
    return std::make_shared<Tap>(std::move(value));
}
