...
namespace mx
{
    namespace core
    {
        ...
        bool OctaveShiftAttributes::fromXElementImpl( std::ostream& message, ::ezxml::XElement& xelement )
        {
            ...
            for( ; it != endIter; ++it )
            {
                if( parseAttribute( message, it, className, isSuccess, type, isTypeFound, "type", &parseUpDownStopContinue ) ) { continue; }
                if( parseAttribute( message, it, className, isSuccess, number, hasNumber, "number" ) ) { continue; }
                if( parseAttribute( message, it, className, isSuccess, size, hasSize, "size" ) ) { continue; }
