...
{
PartGroup::PartGroup()
    : myAttributes(std::make_shared<PartGroupAttributes>()), myGroupName(makeGroupName()), myHasGroupName(false),
      myGroupNameDisplay(makeGroupNameDisplay()), myHasGroupNameDisplay(false),
      myGroupAbbreviation(makeGroupAbbreviation()), myHasGroupAbbreviation(false),
      myGroupAbbreviationDisplay(makeGroupAbbreviationDisplay()), myHasGroupAbbreviationDisplay(false),
      myGroupSymbol(makeGroupSymbol()), myHasGroupSymbol(false), myGroupBarline(makeGroupBarline()),
      myHasGroupBarline(false), myGroupTime(makeGroupTime()), myHasGroupTime(false),
...
{
    return myHasGroupName || myHasGroupNameDisplay || myHasGroupAbbreviation || myHasGroupAbbreviationDisplay ||
           myHasGroupSymbol || myHasGroupBarline || myHasGroupTime || myEditorialGroup->hasContents();
}
...
bool PartGroup::fromXElementImpl(std::ostream &message, ::ezxml::XElement &xelement)
{
    ...
    {
        if (importElement(message, *it, isSuccess, *myGroupName, myHasGroupName))
        {
            continue;
        }
        if (importElement(message, *it, isSuccess, *myGroupNameDisplay, myHasGroupNameDisplay))
        {
            continue;
        }
        if (importElement(message, *it, isSuccess, *myGroupAbbreviation, myHasGroupAbbreviation))
        {
            continue;
        }
        ...
        if (importElement(message, *it, isSuccess, *myGroupSymbol, myHasGroupSymbol))
        {
            continue;
        }
        if (importElement(message, *it, isSuccess, *myGroupBarline, myHasGroupBarline))
        {
            continue;
        }
        if (importElement(message, *it, isSuccess, *myGroupTime, myHasGroupTime))
        {
            continue;
        }
