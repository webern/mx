...
namespace mx
{
    namespace core
    {
        ...
        bool ExtendAttributes::fromXElementImpl( std::ostream& message, ::ezxml::XElement& xelement )
        {
            ...
            for( ; it != endIter; ++it )
            {
                if( parseAttribute( message, it, className, isSuccess, type, hasType, "type", &parseStartStopContinue ) ) { continue; }
