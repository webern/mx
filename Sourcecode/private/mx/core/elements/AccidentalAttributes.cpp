...
bool AccidentalAttributes::fromXElementImpl(std::ostream &message, ::ezxml::XElement &xelement)
{
    ...
    for (; it != endIter; ++it)
    {
        if (parseAttribute(message, it, className, isSuccess, cautionary, hasCautionary, "cautionary", &parseYesNo))
        {
            continue;
        }
        if (parseAttribute(message, it, className, isSuccess, editorial, hasEditorial, "editorial", &parseYesNo))
        {
            continue;
        }
        ...
        if (parseAttribute(message, it, className, isSuccess, bracket, hasBracket, "bracket", &parseYesNo))
        {
            continue;
        }
