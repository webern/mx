...
{
Key::Key()
    : myAttributes(std::make_shared<KeyAttributes>()), myKeyChoice(std::make_shared<KeyChoice>()), myKeyOctaveSet()
{
}
...
std::ostream &Key::streamContents(std::ostream &os, const int indentLevel, bool &isOneLineOnly) const
{
    ...
    for (auto it = myKeyOctaveSet.cbegin(); it != myKeyOctaveSet.cend(); ++it)
    {
...
bool Key::fromXElementImpl(std::ostream &message, ::ezxml::XElement &xelement)
{
    ...
    {
        if (importTraditionalKey(message, it, endIter, isSuccess))
        {
            continue;
        }
        if (importNonTraditionalKey(message, it, endIter, isSuccess))
        {
            continue;
        }
        if (importElementSet(message, it, endIter, isSuccess, "key-octave", myKeyOctaveSet))
        {
            continue;
        }
    }
...
bool Key::importTraditionalKey(std::ostream &message, ::ezxml::XElementIterator &iter,
...
bool Key::importNonTraditionalKey(std::ostream &message, ::ezxml::XElementIterator &iter,
                                  ::ezxml::XElementIterator &endIter, bool &isSuccess)
{
    if (iter == endIter)
    {
    ...
    while (iter != endIter &&
           (iter->getName() == "key-step" || iter->getName() == "key-alter" || iter->getName() == "key-accidental"))
    {
