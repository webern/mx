...
{
Figure::Figure()
    : myPrefix(makePrefix()), myHasPrefix(false), myFigureNumber(makeFigureNumber()), myHasFigureNumber(false),
      mySuffix(makeSuffix()), myHasSuffix(false), myExtend(makeExtend()), myHasExtend(false)
{
}
...
bool Figure::fromXElementImpl(std::ostream &message, ::ezxml::XElement &xelement)
{
    ...
    {
        if (importElement(message, *it, isSuccess, *myPrefix, myHasPrefix))
        {
            continue;
        }
        if (importElement(message, *it, isSuccess, *myFigureNumber, myHasFigureNumber))
        {
            continue;
        }
        if (importElement(message, *it, isSuccess, *mySuffix, myHasSuffix))
        {
            continue;
        }
        if (importElement(message, *it, isSuccess, *myExtend, myHasExtend))
        {
            continue;
        }
    }
