...
namespace core
{
PageLayout::PageLayout() : myPageHeight(makePageHeight()), myPageWidth(makePageWidth()), myPageMarginsSet()
{
}
...
bool PageLayout::fromXElementImpl(std::ostream &message, ::ezxml::XElement &xelement)
{
    ...
    {
        if (importElement(message, *it, isSuccess, *myPageHeight, isPageHeightFound))
        {
            continue;
        }
        if (importElement(message, *it, isSuccess, *myPageWidth, isPageWidthFound))
        {
            continue;
        }
