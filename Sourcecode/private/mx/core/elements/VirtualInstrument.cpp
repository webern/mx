...
VirtualInstrument::VirtualInstrument()
    : myVirtualLibrary(makeVirtualLibrary()), myHasVirtualLibrary(false), myVirtualName(makeVirtualName()),
...
bool VirtualInstrument::fromXElementImpl(std::ostream &message, ::ezxml::XElement &xelement)
{
    bool isSuccess = true;
    ...
    {
        if (importElement(message, *it, isSuccess, *myVirtualLibrary, myHasVirtualLibrary))
        {
            continue;
        }
        if (importElement(message, *it, isSuccess, *myVirtualName, myHasVirtualName))
        {
            continue;
        }
    }
