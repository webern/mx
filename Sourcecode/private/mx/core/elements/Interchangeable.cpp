...
Interchangeable::Interchangeable()
    : myAttributes(std::make_shared<InterchangeableAttributes>()), myTimeRelation(makeTimeRelation()),
      myHasTimeRelation(false), myBeats(makeBeats()), myBeatType(makeBeatType())
{
}
...
bool Interchangeable::fromXElementImpl(std::ostream &message, ::ezxml::XElement &xelement)
{
    ...
    {
        if (importElement(message, *it, isSuccess, *myTimeRelation, myHasTimeRelation))
        {
            continue;
        }
        if (importElement(message, *it, isSuccess, *myBeats, isBeatsFound))
        {
            continue;
        }
        if (importElement(message, *it, isSuccess, *myBeatType, isBeatTypeFound))
        {
            continue;
        }
    }
