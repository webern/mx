...
Properties::Properties()
    : myEditorialGroup(makeEditorialGroup()), myDivisions(makeDivisions()), myHasDivisions(false), myKeySet(),
      myTimeSet(), myStaves(makeStaves()), myHasStaves(false), myPartSymbol(makePartSymbol()), myHasPartSymbol(false),
      myInstruments(makeInstruments()), myHasInstruments(false), myClefSet(), myStaffDetailsSet(), myTransposeSet(),
      myDirectiveSet(), myMeasureStyleSet()
{
}
...
{
    return myEditorialGroup->hasContents() || myHasDivisions || myKeySet.size() > 0 || myTimeSet.size() > 0 ||
           myHasStaves || myHasPartSymbol || myHasInstruments || myClefSet.size() > 0 || myStaffDetailsSet.size() > 0 ||
           myTransposeSet.size() > 0 || myDirectiveSet.size() > 0 || myMeasureStyleSet.size() > 0;
}
...
bool Properties::fromXElementImpl(std::ostream &message, ::ezxml::XElement &xelement)
{
    ...
    for (auto it = xelement.begin(); it != end; ++it)
    {
        ...
        if (importElement(message, *it, isSuccess, *myPartSymbol, myHasPartSymbol))
        {
            continue;
        }
        if (importElement(message, *it, isSuccess, *myInstruments, myHasInstruments))
        {
            continue;
        }
