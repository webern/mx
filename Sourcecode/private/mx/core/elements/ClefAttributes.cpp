...
ClefAttributes::ClefAttributes()
    : number(), additional(YesNo::no), size(), afterBarline(YesNo::no), defaultX(), defaultY(), relativeX(),
      ...
      hasColor(false), hasPrintObject(false)
{
}

bool ClefAttributes::hasValues() const
{
    return hasNumber || hasAdditional || hasSize || hasAfterBarline || hasDefaultX || hasDefaultY || hasRelativeX ||
           hasRelativeY || hasFontFamily || hasFontStyle || hasFontSize || hasFontWeight || hasColor || hasPrintObject;
}
...
bool ClefAttributes::fromXElementImpl(std::ostream &message, ::ezxml::XElement &xelement)
{
    ...
    for (; it != endIter; ++it)
    {
        ...
        if (parseAttribute(message, it, className, isSuccess, additional, hasAdditional, "additional", &parseYesNo))
        {
            continue;
        }
        if (parseAttribute(message, it, className, isSuccess, size, hasSize, "size", &parseSymbolSize))
        {
            continue;
        }
        if (parseAttribute(message, it, className, isSuccess, afterBarline, hasAfterBarline, "after-barline",
                           &parseYesNo))
        {
            continue;
        }
