...
namespace core
{
Root::Root() : myRootStep(makeRootStep()), myRootAlter(makeRootAlter()), myHasRootAlter(false)
{
}
...
bool Root::fromXElementImpl(std::ostream &message, ::ezxml::XElement &xelement)
{
    ...
    {
        if (importElement(message, *it, isSuccess, *myRootStep, isRootStepFound))
        {
            continue;
        }
        if (importElement(message, *it, isSuccess, *myRootAlter, myHasRootAlter))
        {
            continue;
        }
    }
