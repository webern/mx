...
NoteAttributes::NoteAttributes()
    : defaultX(), defaultY(), relativeX(), relativeY(), fontFamily(), fontStyle(FontStyle::normal),
      ...
      printDot(YesNo::no), printSpacing(YesNo::no), printLyric(YesNo::no), dynamics(), endDynamics(), attack(),
      release(), timeOnly(), pizzicato(YesNo::no), hasDefaultX(false), hasDefaultY(false), hasRelativeX(false),
      hasRelativeY(false), hasFontFamily(false), hasFontStyle(false), hasFontSize(false), hasFontWeight(false),
      hasColor(false), hasPrintObject(false), hasPrintDot(false), hasPrintSpacing(false), hasPrintLyric(false),
      hasDynamics(false), hasEndDynamics(false), hasAttack(false), hasRelease(false), hasTimeOnly(false),
      hasPizzicato(false)
{
}
...
bool NoteAttributes::hasValues() const
{
    return hasDefaultX || hasDefaultY || hasRelativeX || hasRelativeY || hasFontFamily || hasFontStyle || hasFontSize ||
           hasFontWeight || hasColor || hasPrintObject || hasPrintDot || hasPrintSpacing || hasPrintLyric ||
           hasDynamics || hasEndDynamics || hasAttack || hasRelease || hasTimeOnly || hasPizzicato;
}
...
bool NoteAttributes::fromXElementImpl(std::ostream &message, ::ezxml::XElement &xelement)
{
    ...
    for (; it != endIter; ++it)
    {
        ...
        if (parseAttribute(message, it, className, isSuccess, printObject, hasPrintObject, "print-object", &parseYesNo))
        {
            continue;
        }
        if (parseAttribute(message, it, className, isSuccess, printDot, hasPrintDot, "print-dot", &parseYesNo))
        {
            continue;
        }
        if (parseAttribute(message, it, className, isSuccess, printSpacing, hasPrintSpacing, "print-spacing",
                           &parseYesNo))
        {
            continue;
        }
        if (parseAttribute(message, it, className, isSuccess, printLyric, hasPrintLyric, "print-lyric", &parseYesNo))
        {
            continue;
        }
        ...
        if (parseAttribute(message, it, className, isSuccess, endDynamics, hasEndDynamics, "end-dynamics"))
        {
            continue;
        }
        if (parseAttribute(message, it, className, isSuccess, attack, hasAttack, "attack"))
        {
            continue;
        }
        if (parseAttribute(message, it, className, isSuccess, release, hasRelease, "release"))
        {
            continue;
        }
