...
#define MXTEST_IMPORT_ROUNDTRIP(GROUPNAME, FNAME_PART, FEXT, IMPORT_DIR)                                               \
    ...
    {                                                                                                                  \
      public:                                                                                                          \
        GROUPNAME##FNAME_PART##Test() : Test("TEST(" #GROUPNAME ", " #FNAME_PART ")", __FILE__, __LINE__)              \
        {                                                                                                              \
        }                                                                                                              \
    ...
    {                                                                                                                  \
        mxtest::ImportRoundTripTest test(#FNAME_PART, #FEXT, #GROUPNAME, __FILE__, __LINE__, IMPORT_DIR);              \
                                                                                                                       \
...
{
  public:
    ImportRoundTripTest(const char *const paramInputFileNamePart, const char *const paramInputFileExtensionPart,
  ...
  private:
    ...
    bool checkEqual(std::ostream &msg, const std::string &inExpected, const std::string &inActual,
...
{
  public:
    ImportTestCpul(mxtest::MxFile inTestFile, std::string inTestName, std::string inTestCppFileName,
    ...
    {
        mxtest::ImportRoundTripTest test{testFileNamePart().c_str(), testFileExtension().c_str(),
                                         testSubdirectory().c_str(), testFileName().c_str(),
                                         getCppFileLineNumber(),     testSubdirectory().c_str()};
