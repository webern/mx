...
TEST( Test01, TripleTongue )
{
	std::string indentString( INDENT );
	TripleTongue object1;
	TripleTongue object2;
	...
	std::string expected = R"(<triple-tongue/>)";
	std::string actual = default_constructed.str();
	CHECK_EQUAL( expected, actual )
	expected = indentString+indentString+R"(<triple-tongue default-x="0.1" font-style="italic" font-weight="bold"/>)";
