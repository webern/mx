...
TEST( Test01, DoubleTongue )
{
	std::string indentString( INDENT );
	DoubleTongue object1;
	DoubleTongue object2;
	...
	std::string expected = R"(<double-tongue/>)";
	std::string actual = default_constructed.str();
	CHECK_EQUAL( expected, actual )
	expected = indentString+indentString+R"(<double-tongue default-x="0.1" font-style="italic" font-weight="bold"/>)";
