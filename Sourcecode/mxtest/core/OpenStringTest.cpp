...
TEST( Test01, OpenString )
{
	...
	std::string expected = R"(<open-string/>)";
	std::string actual = default_constructed.str();
	CHECK_EQUAL( expected, actual )
	expected = indentString+indentString+R"(<open-string default-x="0.1" font-style="italic" font-weight="bold"/>)";
