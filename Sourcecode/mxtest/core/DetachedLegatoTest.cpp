...
TEST( Test01, DetachedLegato )
{
	std::string indentString( INDENT );
	DetachedLegato object1;
	DetachedLegato object2;
	...
	std::string expected = R"(<detached-legato/>)";
	std::string actual = default_constructed.str();
	CHECK_EQUAL( expected, actual )
	expected = indentString+indentString+R"(<detached-legato default-x="0.1" font-style="italic" font-weight="bold"/>)";
