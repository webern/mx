...
namespace mx
{
    namespace core
    {
        ...
        bool TremoloAttributes::fromXElement( std::ostream& message, xml::XElement& xelement )
        {
            ...
            for( ; it != endIter; ++it )
            {
                if( parseAttribute( message, it, className, isSuccess, type, hasType, "type", &parseStartStopSingle ) ) { continue; }
