...
namespace mx
{
    namespace core
    {
        ...
        bool FermataAttributes::fromXElement( std::ostream& message, xml::XElement& xelement )
        {
            ...
            for( ; it != endIter; ++it )
            {
                if( parseAttribute( message, it, className, isSuccess, type, hasType, "type", &parseUprightInverted ) ) { continue; }
