...
namespace mx
{
    namespace impl
    {
        ...
        api::LineData getLineData( const ATTRIBUTES_TYPE& inAttributes )
        {
            ...
            if( checkHasLineType( &inAttributes ) )
            {
                lineData.lineType = checkLineType( &inAttributes );
            }
            
            if( checkHasLineEnd( &inAttributes ) )
            {
                lineData.lineEnd = converter.convert( checkLineEnd( &inAttributes ) );
            }
