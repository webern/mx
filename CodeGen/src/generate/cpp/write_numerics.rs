...
impl Writer {
    ...
    fn open_integers_h(&self) -> std::io::Result<impl Write> {
        ...
        l!(&mut f, 2, "public:")?;
        l!(&mut f, 3, "Integer();")?;
        l!(&mut f, 3, "explicit Integer( IntType value );")?;
        l!(&mut f, 3, "virtual ~Integer() = default;")?;
        l!(&mut f, 3, "virtual IntType getValue() const final;")?;
        l!(&mut f, 3, "virtual void setValue( IntType value );")?;
        l!(&mut f, 3, "virtual void parse( const std::string& value ) final;")?;
        l!(&mut f, 2, "protected:")?;
        l!(&mut f, 3, "IntType myValue;")?;
        l!(&mut f, 2, "}};")?;
        l!(&mut f, 2, "")?;
    ...
    fn open_integers_cpp(&self) -> std::io::Result<impl Write> {
        ...
        l!(&mut f, 2, "{{")?;
        l!(&mut f, 3, "std::stringstream ss( value );")?;
        l!(&mut f, 3, "IntType temp = 0;")?;
        l!(&mut f, 3, "if ((ss >> temp).fail() || !(ss >> std::ws).eof())")?;
        l!(&mut f, 4, "return;")?;
        l!(&mut f, 3, "}}")?;
        l!(&mut f, 3, "setValue( temp );")?;
        l!(&mut f, 2, "}}")?;
        l!(&mut f, 0, "")?;
        Ok(f)
    }
