...
MxOffset *mx_offset_parse(xmlNodePtr el) {
    ...
    for (xmlAttrPtr a = el->properties; a; a = a->next) {
        ...
        (void)s;
        if (strcmp(aname, "sound") == 0) {
            m->has_sound = true;
            m->sound = mx_yes_no_parse(s);
        } else {
...
xmlNodePtr mx_offset_serialize(const MxOffset *m, xmlNodePtr parent, const char *tag) {
    ...
    {
        char *s = mx_divisions_to_string(m->value);
        xmlAddChild(el, xmlNewText(BAD_CAST s));
        free(s);
    }
    if (m->has_sound) {
        xmlSetProp(el, BAD_CAST "sound", BAD_CAST mx_yes_no_to_string(m->sound));
    }
    return el;
}
