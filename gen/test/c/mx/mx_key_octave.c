...
MxKeyOctave *mx_key_octave_parse(xmlNodePtr el) {
    MxKeyOctave *m = calloc(1, sizeof(*m));
    if (!m)
        abort();
    for (xmlAttrPtr a = el->properties; a; a = a->next) {
        ...
        if (strcmp(aname, "number") == 0) {
            m->has_number = true;
            m->number = mx_parse_int(s);
        } else if (strcmp(aname, "cancel") == 0) {
            m->has_cancel = true;
            m->cancel = mx_yes_no_parse(s);
        } else {
            mx_error_set("unknown attribute \"%s\" on <%s>", aname,
                         (const char *)el->name);
            xmlFree(avalue);
            mx_key_octave_free(m);
            return NULL;
        }
        xmlFree(avalue);
    }
...
xmlNodePtr mx_key_octave_serialize(const MxKeyOctave *m, xmlNodePtr parent, const char *tag) {
    ...
    {
        char *s = mx_octave_to_string(m->value);
        xmlAddChild(el, xmlNewText(BAD_CAST s));
        free(s);
    }
    if (m->has_number) {
        char *s = mx_format_int(m->number);
        xmlSetProp(el, BAD_CAST "number", BAD_CAST s);
        free(s);
    }
    if (m->has_cancel) {
        xmlSetProp(el, BAD_CAST "cancel", BAD_CAST mx_yes_no_to_string(m->cancel));
    }
    return el;
}
