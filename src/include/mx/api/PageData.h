...
{
  public:
    inline explicit PageData(Bool inNewPage = Bool::unspecified, PageLayoutData inPageLayoutData = PageLayoutData{},
                             std::optional<std::string> inPageNumber = std::nullopt)
        : newPage{inNewPage}, pageLayoutData{inPageLayoutData}, pageNumber{std::move(inPageNumber)}
    {
    }
