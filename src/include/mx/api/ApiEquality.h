...
namespace api
{
template <typename T> inline bool areVectorsEqual(const std::vector<T> &lhs, const std::vector<T> &rhs)
{
...
template <typename T> inline bool areIntMapsEqual(const std::map<int, T> &lhs, const std::map<int, T> &rhs)
{
    if (lhs.size() != rhs.size())
    {
        return false;
...
template <typename T> inline bool areEqual(const T &lhs, const T &rhs)
{
    return lhs == rhs;
}
...
#define MXAPI_NOT_EQUALS_AND_VECTORS(mxapiClassName)                                                                   \
    ...
    inline bool operator==(const std::vector<mxapiClassName> &lhs, const std::vector<mxapiClassName> &rhs)             \
    {                                                                                                                  \
        return areVectorsEqual<mxapiClassName>(lhs, rhs);                                                              \
    }                                                                                                                  \
    inline bool operator!=(const std::vector<mxapiClassName> &lhs, const std::vector<mxapiClassName> &rhs)             \
    {                                                                                                                  \
        return !areVectorsEqual<mxapiClassName>(lhs, rhs);                                                             \
