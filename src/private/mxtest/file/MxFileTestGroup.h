...
template <typename MX_FILE_TEST_DERIVITAVE_TYPE> class MxFileTestGroup
{
  public:
    ...
    MxFileTestGroup(int maxFileSizeBytes, std::string testName, std::string testCppFileName, int testCppFileLineNumber)
    {
        auto files = MxFileRepository::getTestFiles(maxFileSizeBytes);
        for (auto &file : files)
        {
            std::shared_ptr<MxFileTest> testPtr{
                new MX_FILE_TEST_DERIVITAVE_TYPE{file, testName, testCppFileName, testCppFileLineNumber}};
    ...
    MxFileTestGroup(int maxFileSizeBytes, std::vector<std::string> fileList, MxFileTestListType listType,
                    std::string testName, std::string testCppFileName, int testCppFileLineNumber)
    {
