...
ImportRoundTripTest::ImportRoundTripTest(const char *const paramInputFileNamePart,
                                         const char *const paramInputFileExtensionPart,
                                         const char *const paramInputSuiteName, const char *const paramTestCppFileName,
                                         const int paramTestCppLineNum, const char *const paramResourceDirectoryPath)
    : myInputFileNamePart(paramInputFileNamePart), myInputFileExtensionPart(paramInputFileExtensionPart),
      myInputSuiteName(paramInputSuiteName), myTestCppFileName(paramTestCppFileName),
      myTestCppLineNum(paramTestCppLineNum), myResourceDirectoryPath(paramResourceDirectoryPath), myTestTimer(),
      myXdocLoadTimer(), myMxDeserializeTimer(), myMxReserializeTimer(), myTestCompareTimer(), myXdocWriteTimer(),
...
bool ImportRoundTripTest::loadIntoMxClasses(std::ostream &msg, const ::ezxml::XDocPtr &inXdoc,
                                            mx::core::DocumentPtr &outMx)
{
    if (myIsMxBypassed)
    {
        return true;
...
bool ImportRoundTripTest::reserializeToXDoc(std::ostream &msg, const mx::core::DocumentPtr &inMx,
                                            ::ezxml::XDocPtr &outXDoc)
{
    if (myIsMxBypassed)
    {
        return true;
    ...
    try
    {
        ...
        // because attribute order doesn't matter in xml, we need to sort them alphabetically before and after round
        // trip
...
bool ImportRoundTripTest::checkEqual(std::ostream &msg, const std::string &inExpected, const std::string &inActual,
                                     double &outPercentMatch)
{
    outPercentMatch = 0.0;
...
bool ImportRoundTripTest::writeErrorFiles(bool isSuccess, const std::string &expectedContents,
