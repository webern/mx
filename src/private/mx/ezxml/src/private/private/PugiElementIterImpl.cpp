...
PugiElementIterImpl::PugiElementIterImpl()
    : myIter(), myIterParent(), myXDoc(), mySkipProcessingInstructions{true}, myReturnableElement{nullptr}
{
}

PugiElementIterImpl::PugiElementIterImpl(const pugi::xml_node_iterator &iter, const pugi::xml_node &iterParent,
                                         const XDocCPtr &parentDoc)
    : myIter(iter), myIterParent(iterParent), myXDoc(parentDoc), mySkipProcessingInstructions{true},
      myReturnableElement{nullptr}
{
}

PugiElementIterImpl::PugiElementIterImpl(const PugiElementIterImpl &inOther)
    : myIter(inOther.myIter), myIterParent(inOther.myIterParent), myXDoc(inOther.myXDoc),
      mySkipProcessingInstructions{inOther.mySkipProcessingInstructions},
      myReturnableElement{inOther.myReturnableElement == nullptr
                              ? nullptr
...
PugiElementIterImpl &PugiElementIterImpl::operator=(const PugiElementIterImpl &inOther)
{
    ...
    mySkipProcessingInstructions = inOther.mySkipProcessingInstructions;
    myReturnableElement =
        inOther.myReturnableElement == nullptr ? nullptr : std::make_unique<PugiElement>(*inOther.myReturnableElement);
    return *this;
}
