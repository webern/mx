...
PugiDoc::PugiDoc()
    : myDoc(), myXmlVersion(DEFAULT_XML_VERSION), myEncoding(DEFAULT_ENCODING), myIsStandalone(false),
      myDoWriteBom(false)
{
...
void PugiDoc::setHasStandaloneAttribute(bool value)
{
    ...
    {
        auto xmlDeclarationNode = getXmlDeclarationNode();
        for (auto it = xmlDeclarationNode.attributes_begin(); it != xmlDeclarationNode.attributes_end(); ++it)
        {
...
pugi::xml_node PugiDoc::getDoctypeNode() const
{
    ...
    if (childrenIter == myDoc.end() || childrenIter->type() != pugi::node_doctype)
    {
        return pugi::xml_node{};
    }
    else
