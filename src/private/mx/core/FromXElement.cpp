...
namespace core
{
bool importElement(std::ostream &message, ::ezxml::XElement &xelement, bool &isSuccess, ElementInterface &element,
                   bool &isMatch)
{
...
bool isEndWithDecrementIfNeeded(::ezxml::XElementIterator &iter, ::ezxml::XElementIterator &endIter,
...
void importGroup(std::ostream &message, ::ezxml::XElementIterator &iter, ::ezxml::XElementIterator &endIter,
                 bool &isSuccess, EditorialGroupPtr &group)
{
    ...
    checkSetGroupMemberAndIncrementIter(message, iter, endIter, isSuccess, doDecrementIter, group,
                                        &EditorialGroup::getFootnote, &EditorialGroup::setHasFootnote);
    if (isEndWithDecrementIfNeeded(iter, endIter, doDecrementIter))
    {
        return;
    }

    checkSetGroupMemberAndIncrementIter(message, iter, endIter, isSuccess, doDecrementIter, group,
                                        &EditorialGroup::getLevel, &EditorialGroup::setHasLevel);
    if (isEndWithDecrementIfNeeded(iter, endIter, doDecrementIter))
    {
        return;
    }
...
void importGroup(std::ostream &message, ::ezxml::XElementIterator &iter, ::ezxml::XElementIterator &endIter,
                 bool &isSuccess, EditorialVoiceGroupPtr &group)
{
    bool doDecrementIter = false;
    if (iter == endIter)
    {
    ...
    checkSetGroupMemberAndIncrementIter(message, iter, endIter, isSuccess, doDecrementIter, group,
                                        &EditorialVoiceGroup::getFootnote, &EditorialVoiceGroup::setHasFootnote);
    if (isEndWithDecrementIfNeeded(iter, endIter, doDecrementIter))
    {
        return;
    }
    ...
    checkSetGroupMemberAndIncrementIter(message, iter, endIter, isSuccess, doDecrementIter, group,
                                        &EditorialVoiceGroup::getLevel, &EditorialVoiceGroup::setHasLevel);
    if (isEndWithDecrementIfNeeded(iter, endIter, doDecrementIter))
    {
        return;
    }
    ...
    checkSetGroupMemberAndIncrementIter(message, iter, endIter, isSuccess, doDecrementIter, group,
                                        &EditorialVoiceGroup::getVoice, &EditorialVoiceGroup::setHasVoice);
    if (isEndWithDecrementIfNeeded(iter, endIter, doDecrementIter))
    {
        return;
    }
...
void importGroup(std::ostream &message, ::ezxml::XElementIterator &iter, ::ezxml::XElementIterator &endIter,
                 bool &isSuccess, EditorialVoiceDirectionGroupPtr &group)
{
    bool doDecrementIter = false;
    if (iter == endIter)
    {
    ...
    checkSetGroupMemberAndIncrementIter(message, iter, endIter, isSuccess, doDecrementIter, group,
                                        &EditorialVoiceDirectionGroup::getFootnote,
                                        &EditorialVoiceDirectionGroup::setHasFootnote);
    if (isEndWithDecrementIfNeeded(iter, endIter, doDecrementIter))
    {
        return;
    }
    ...
    checkSetGroupMemberAndIncrementIter(message, iter, endIter, isSuccess, doDecrementIter, group,
                                        &EditorialVoiceDirectionGroup::getLevel,
                                        &EditorialVoiceDirectionGroup::setHasLevel);
    if (isEndWithDecrementIfNeeded(iter, endIter, doDecrementIter))
    {
        return;
    }
    ...
    checkSetGroupMemberAndIncrementIter(message, iter, endIter, isSuccess, doDecrementIter, group,
                                        &EditorialVoiceDirectionGroup::getVoice,
                                        &EditorialVoiceDirectionGroup::setHasVoice);
    if (isEndWithDecrementIfNeeded(iter, endIter, doDecrementIter))
    {
        return;
    }
...
void importGroup(std::ostream &message, ::ezxml::XElementIterator &iter, ::ezxml::XElementIterator &endIter,
                 bool &isSuccess, LayoutGroupPtr &group)
{
    ...
    checkSetGroupMemberAndIncrementIter(message, iter, endIter, isSuccess, isIterIncremented, group,
                                        &LayoutGroup::getPageLayout, &LayoutGroup::setHasPageLayout);
    if (isEndWithDecrementIfNeeded(iter, endIter, isIterIncremented))
    {
        return;
    }

    checkSetGroupMemberAndIncrementIter(message, iter, endIter, isSuccess, isIterIncremented, group,
                                        &LayoutGroup::getSystemLayout, &LayoutGroup::setHasSystemLayout);
    if (isEndWithDecrementIfNeeded(iter, endIter, isIterIncremented))
    {
        return;
    }

    checkSetGroupMemberSetAndIncrementIter(message, iter, endIter, isSuccess, isIterIncremented, "staff-layout", group,
                                           &LayoutGroup::getStaffLayoutSet, &LayoutGroup::addStaffLayout,
                                           &LayoutGroup::removeStaffLayout);
    if (isEndWithDecrementIfNeeded(iter, endIter, isIterIncremented))
    {
        return;
    }
...
void importGroup(std::ostream &message, ::ezxml::XElementIterator &iter, ::ezxml::XElementIterator &endIter,
                 bool &isSuccess, BeatUnitGroupPtr &group)
{
    bool isIterIncremented = false;
    if (iter == endIter)
    {
    ...
    checkSetGroupMemberAndIncrementIter(message, iter, endIter, isSuccess, isIterIncremented, group,
                                        &BeatUnitGroup::getBeatUnit, dummy);
    if (isEndWithDecrementIfNeeded(iter, endIter, isIterIncremented))
    {
        return;
    }

    checkSetGroupMemberSetAndIncrementIter(message, iter, endIter, isSuccess, isIterIncremented, "beat-unit-dot", group,
                                           &BeatUnitGroup::getBeatUnitDotSet, &BeatUnitGroup::addBeatUnitDot,
                                           &BeatUnitGroup::removeBeatUnitDot);
    if (isEndWithDecrementIfNeeded(iter, endIter, isIterIncremented))
    {
        return;
    }
...
void importGroup(std::ostream &message, ::ezxml::XElementIterator &iter, ::ezxml::XElementIterator &endIter,
                 bool &isSuccess, MetronomeRelationGroupPtr &group, bool &hasGroup)
{
    bool isIterIncremented = false;
    if (iter == endIter)
    {
    ...
    checkSetGroupMemberAndIncrementIter(message, iter, endIter, isSuccess, isIterIncremented, group,
                                        &MetronomeRelationGroup::getMetronomeRelation, isFound);
    hasGroup |= isFound;
    if (isEndWithDecrementIfNeeded(iter, endIter, isIterIncremented))
    {
        return;
    ...
    checkSetGroupMemberAndIncrementIter(message, iter, endIter, isSuccess, isIterIncremented, group,
                                        &MetronomeRelationGroup::getMetronomeNote, isFound);
    hasGroup |= isFound;
    if (isEndWithDecrementIfNeeded(iter, endIter, isIterIncremented))
    {
        return;
...
void importGroup(std::ostream &message, ::ezxml::XElementIterator &iter, ::ezxml::XElementIterator &endIter,
                 bool &isSuccess, DisplayStepOctaveGroupPtr &group, bool &hasGroup)
{
    hasGroup = false;
    bool isIterIncremented = false;
    if (iter == endIter)
...
void importGroup(std::ostream &message, ::ezxml::XElementIterator &iter, ::ezxml::XElementIterator &endIter,
                 bool &isSuccess, SyllabicTextGroupPtr &group)
{
    bool isIterIncremented = false;
    if (iter == endIter)
    {
    ...
    checkSetGroupMemberAndIncrementIter(message, iter, endIter, isSuccess, isIterIncremented, group,
                                        &SyllabicTextGroup::getSyllabic, &SyllabicTextGroup::setHasSyllabic);
    if (isEndWithDecrementIfNeeded(iter, endIter, isIterIncremented))
    {
        return;
    }
    ...
    bool hasText = false;
    checkSetGroupMemberAndIncrementIter(message, iter, endIter, isSuccess, isIterIncremented, group,
                                        &SyllabicTextGroup::getText, hasText);
    if (isEndWithDecrementIfNeeded(iter, endIter, isIterIncremented))
    {
        return;
    }
    ...
    {
        isSuccess = false;
        message << "SyllabicTextGroup: 'text' was not found where expected, '" << iter->getName()
                << "' was found instead" << std::endl;
    }

    while (iter != endIter &&
           (iter->getName() == "elision" || iter->getName() == "syllabic" || iter->getName() == "text"))
    {
        ...
        if (iter->getName() == "elision")
        {
            ...
            isSuccess &=
                elisionSyllabicTextGroup->getElisionSyllabicGroup()->getElision()->fromXElement(message, *iter);
        ...
        if (iter->getName() == "syllabic")
        {
            ...
            isSuccess &=
                elisionSyllabicTextGroup->getElisionSyllabicGroup()->getSyllabic()->fromXElement(message, *iter);
            elisionSyllabicTextGroup->setHasElisionSyllabicGroup(true);
            ++iter;
            isIterIncremented = true;
        }
    ...
    checkSetGroupMemberAndIncrementIter(message, iter, endIter, isSuccess, isIterIncremented, group,
                                        &SyllabicTextGroup::getExtend, &SyllabicTextGroup::setHasExtend);
    if (isEndWithDecrementIfNeeded(iter, endIter, isIterIncremented))
    {
        return;
    }
...
void importGroup(std::ostream &message, ::ezxml::XElementIterator &iter, ::ezxml::XElementIterator &endIter,
                 bool &isSuccess, TimeSignatureGroupPtr &group)
{
    bool isIterIncremented = false;
    if (iter == endIter)
    {
    ...
    if (!isBeatTypeFound)
    {
        message << "importGroup: the time-signature group requires a 'beat-type' element but it was not found"
                << std::endl;
        isSuccess = false;
    }
...
void importGroup(std::ostream &message, ::ezxml::XElementIterator &iter, ::ezxml::XElementIterator &endIter,
                 bool &isSuccess, MidiInstrumentPtr &group)
{
    bool isIterIncremented = false;
    if (iter == endIter)
    {
...
bool parseAttribute(std::ostream &message, ::ezxml::XAttributeIterator &iter, const std::string &className,
                    bool &isSuccess, XsString &assignmentTarget, bool &hasAttribute, const char *const attributeName)
{
    MX_UNUSED(message);
    MX_UNUSED(className);
    MX_UNUSED(isSuccess);
...
bool parseAttribute(std::ostream &message, ::ezxml::XAttributeIterator &iter, const std::string &className,
                    bool &isSuccess, Decimal &assignmentTarget, bool &hasAttribute, const char *const attributeName)
{
    MX_UNUSED(message);
    MX_UNUSED(className);
    MX_UNUSED(isSuccess);
...
bool parseAttribute(std::ostream &message, ::ezxml::XAttributeIterator &iter, const std::string &className,
                    bool &isSuccess, Integer &assignmentTarget, bool &hasAttribute, const char *const attributeName)
{
    MX_UNUSED(message);
    MX_UNUSED(className);
    MX_UNUSED(isSuccess);
...
bool parseAttribute(std::ostream &message, ::ezxml::XAttributeIterator &iter, const std::string &className,
                    bool &isSuccess, Color &assignmentTarget, bool &hasAttribute, const char *const attributeName)
{
    if (iter->getName() != attributeName)
    {
        return false;
...
bool parseAttribute(std::ostream &message, ::ezxml::XAttributeIterator &iter, const std::string &className,
                    bool &isSuccess, Date &assignmentTarget, bool &hasAttribute, const char *const attributeName)
{
    if (iter->getName() != attributeName)
    {
        return false;
...
bool parseAttribute(std::ostream &message, ::ezxml::XAttributeIterator &iter, const std::string &className,
                    bool &isSuccess, FontSize &assignmentTarget, bool &hasAttribute, const char *const attributeName)
{
    MX_UNUSED(message);
    MX_UNUSED(className);
    MX_UNUSED(isSuccess);
...
bool parseAttribute(std::ostream &message, ::ezxml::XAttributeIterator &iter, const std::string &className,
                    bool &isSuccess, NumberOrNormal &assignmentTarget, bool &hasAttribute,
...
bool parseAttribute(std::ostream &message, ::ezxml::XAttributeIterator &iter, const std::string &className,
                    bool &isSuccess, PositiveIntegerOrEmpty &assignmentTarget, bool &hasAttribute,
                    const char *const attributeName)
{
    MX_UNUSED(message);
    MX_UNUSED(className);
...
bool parseAttribute(std::ostream &message, ::ezxml::XAttributeIterator &iter, const std::string &className,
                    bool &isSuccess, YesNoNumber &assignmentTarget, bool &hasAttribute, const char *const attributeName)
{
    if (iter->getName() != attributeName)
    {
        return false;
...
bool parseAttribute(std::ostream &message, ::ezxml::XAttributeIterator &iter, const std::string &className,
                    bool &isSuccess, CommaSeparatedText &assignmentTarget, bool &hasAttribute,
                    const char *const attributeName)
{
    MX_UNUSED(message);
    MX_UNUSED(className);
...
bool parseAttribute(std::ostream &message, ::ezxml::XAttributeIterator &iter, const std::string &className,
                    bool &isSuccess, DistanceType &assignmentTarget, bool &hasAttribute,
                    const char *const attributeName)
{
    MX_UNUSED(message);
    MX_UNUSED(className);
...
bool parseAttribute(std::ostream &message, ::ezxml::XAttributeIterator &iter, const std::string &className,
                    bool &isSuccess, LineWidthType &assignmentTarget, bool &hasAttribute,
                    const char *const attributeName)
{
    MX_UNUSED(message);
    MX_UNUSED(className);
...
bool parseAttribute(std::ostream &message, ::ezxml::XAttributeIterator &iter, const std::string &className,
                    bool &isSuccess, CommaSeparatedListOfPositiveIntegers &assignmentTarget, bool &hasAttribute,
                    const char *const attributeName)
{
    MX_UNUSED(message);
    MX_UNUSED(className);
