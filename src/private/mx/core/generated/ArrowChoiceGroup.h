// Code generated by gen from musicxml-4.0-ed15c23. DO NOT EDIT.

#pragma once

#include "mx/core/generated/ArrowDirection.h"
#include "mx/core/generated/ArrowStyle.h"
#include "mx/core/generated/Empty.h"

#include <optional>
#include <span>
#include <string>
#include <vector>

namespace pugi
{
class xml_node;
}

namespace mx::core
{

class ParseContext;

/// A shared content group (synthesized from an anonymous
/// particle of the schema): transparent on the wire, its
/// fields serialize directly into the referencing element (plan §2.3).
class ArrowChoiceGroup final
{
  public:
    const ArrowDirection &arrowDirection() const noexcept;
    void setArrowDirection(ArrowDirection value);
    const std::optional<ArrowStyle> &arrowStyle() const noexcept;
    void setArrowStyle(std::optional<ArrowStyle> value);
    bool arrowhead() const noexcept;
    void setArrowhead(bool value) noexcept;
    bool operator==(const ArrowChoiceGroup &other) const = default;

  private:
    ArrowDirection m_arrowDirection{};
    std::optional<ArrowStyle> m_arrowStyle;
    bool m_arrowhead{false};
};

/// Consumes matching siblings starting at `cursor` (which advances); `el`
/// is the enclosing element, for error paths.
ArrowChoiceGroup parseArrowChoiceGroup(pugi::xml_node el, pugi::xml_node &cursor, const ParseContext &context);

void serializeArrowChoiceGroup(const ArrowChoiceGroup &v, pugi::xml_node el);

} // namespace mx::core
