...
ScoreTimewisePtr partwiseTimewise(const ScorePartwisePtr &inScore)
{
    ...
    for (Intz currentMeasureIndex = 0; currentMeasureIndex < measureCount; ++currentMeasureIndex)
    {
        ...
        for (auto sp : partlist)
        {
            ...
            auto partIter = std::find_if(inScore->getPartwisePartSet().cbegin(), inScore->getPartwisePartSet().cend(),
                                         [&](const PartwisePartPtr &searchingForThis) {
                                             return searchingForThis->getAttributes()->id.getValue() ==
                                                    sp->getAttributes()->id.getValue();
                                         });
            if (partIter != inScore->getPartwisePartSet().cend())
            {
                ...
                for (Intz x = 0; x < currentMeasureIndex; ++measureIter, ++x)
                {
                }
...
ScorePartwisePtr timewisePartwise(const ScoreTimewisePtr &inScore)
{
    ...
    for (auto p : score_new->getPartwisePartSet())
    {
        ...
        for (auto m : inScore->getTimewiseMeasureSet())
        {
            ...
            auto partIter = std::find_if(m->getTimewisePartSet().cbegin(), m->getTimewisePartSet().cend(),
                                         [&](const TimewisePartPtr &searchingForThis) {
                                             return searchingForThis->getAttributes()->id.getValue() ==
                                                    p->getAttributes()->id.getValue();
                                         });
