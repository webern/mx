...
{
    return hasType || hasNumber || hasBracket || hasShowNumber || hasShowType || hasLineShape || hasDefaultX ||
           hasDefaultY || hasRelativeX || hasRelativeY || hasPlacement;
}
...
bool TupletAttributes::fromXElementImpl(std::ostream &message, ::ezxml::XElement &xelement)
{
    ...
    for (; it != endIter; ++it)
    {
        if (parseAttribute(message, it, className, isSuccess, type, isTypeFound, "type", &parseStartStop))
        {
            continue;
        }
        ...
        if (parseAttribute(message, it, className, isSuccess, showNumber, hasShowNumber, "show-number",
                           &parseShowTuplet))
        {
            continue;
        }
        if (parseAttribute(message, it, className, isSuccess, showType, hasShowType, "show-type", &parseShowTuplet))
        {
            continue;
        }
