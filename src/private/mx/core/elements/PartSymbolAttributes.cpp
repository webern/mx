...
bool PartSymbolAttributes::fromXElementImpl(std::ostream &message, ::ezxml::XElement &xelement)
{
    ...
    for (; it != endIter; ++it)
    {
        if (parseAttribute(message, it, className, isSuccess, topStaff, hasTopStaff, "top-staff"))
        {
            continue;
        }
        if (parseAttribute(message, it, className, isSuccess, bottomStaff, hasBottomStaff, "bottom-staff"))
        {
            continue;
        }
