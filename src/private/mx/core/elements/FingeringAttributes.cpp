...
bool FingeringAttributes::fromXElementImpl(std::ostream &message, ::ezxml::XElement &xelement)
{
    ...
    for (; it != endIter; ++it)
    {
        ...
        if (parseAttribute(message, it, className, isSuccess, alternate, hasAlternate, "alternate", &parseYesNo))
        {
            continue;
        }
