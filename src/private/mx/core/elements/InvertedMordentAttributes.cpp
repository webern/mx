...
bool InvertedMordentAttributes::hasValues() const
{
    return hasDefaultX || hasDefaultY || hasRelativeX || hasRelativeY || hasFontFamily || hasFontStyle || hasFontSize ||
           ...
           hasAccelerate || hasBeats || hasSecondBeat || hasLastBeat || hasLong || hasApproach || hasDeparture;
}
...
bool InvertedMordentAttributes::fromXElementImpl(std::ostream &message, ::ezxml::XElement &xelement)
{
    ...
    for (; it != endIter; ++it)
    {
        ...
        if (parseAttribute(message, it, className, isSuccess, trillStep, hasTrillStep, "trill-step", &parseTrillStep))
        {
            continue;
        }
        if (parseAttribute(message, it, className, isSuccess, twoNoteTurn, hasTwoNoteTurn, "two-note-turn",
                           &parseTwoNoteTurn))
        {
            continue;
        }
        ...
        if (parseAttribute(message, it, className, isSuccess, beats, hasBeats, "beats"))
        {
            continue;
        }
        if (parseAttribute(message, it, className, isSuccess, secondBeat, hasSecondBeat, "second-beat"))
        {
            continue;
        }
        if (parseAttribute(message, it, className, isSuccess, lastBeat, hasLastBeat, "last-beat"))
        {
            continue;
        }
        if (parseAttribute(message, it, className, isSuccess, long_, hasLong, "long", &parseYesNo))
        {
            continue;
        }
        if (parseAttribute(message, it, className, isSuccess, approach, hasApproach, "approach", &parseAboveBelow))
        {
            continue;
        }
        if (parseAttribute(message, it, className, isSuccess, departure, hasDeparture, "departure", &parseAboveBelow))
        {
            continue;
        }
    }
