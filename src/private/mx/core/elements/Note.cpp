...
{
Note::Note()
    : myAttributes(nullptr), myNoteChoice(nullptr), myInstrument(nullptr), myHasInstrument(false),
      myEditorialVoiceGroup(nullptr), myType(nullptr), myHasType(false), myDotSet(), myAccidental(nullptr),
      myHasAccidental(false), myTimeModification(nullptr), myHasTimeModification(false), myStem(nullptr),
      myHasStem(false), myNotehead(nullptr), myHasNotehead(false), myNoteheadText(nullptr), myHasNoteheadText(false),
      myStaff(nullptr), myHasStaff(false), myBeamSet(), myNotationsSet(), myLyricSet(), myPlay(nullptr),
      myHasPlay(false)
{
}
...
bool Note::fromXElementImpl(std::ostream &message, ::ezxml::XElement &xelement)
{
    ...
    for (auto it = xelement.begin(); it != xelement.end(); ++it)
    {
        ...
        if (elementName == "pitch" || elementName == "unpitched" || elementName == "rest" || elementName == "chord" ||
            elementName == "grace" || elementName == "cue")
        {
        ...
        else if (elementName == "footnote" || elementName == "level" || elementName == "voice")
        {
...
bool Note::parseFullNoteGroup(std::ostream &message, ::ezxml::XElement &noteElement, ::ezxml::XElementIterator &iter,
                              FullNoteGroupPtr &outFullNoteGroup)
{
    ...
    {
        message << "Note: parseFullNoteGroup encountered unexpected element '" << noteChoiceElementName << "'"
                << std::endl;
        isSuccess = false;
        ++iter;
    }
...
bool Note::parseEditorialVoiceGroup(std::ostream &message, ::ezxml::XElement &noteElement,
                                    ::ezxml::XElementIterator &iter)
{
    bool isSuccess = true;
    bool isIterIncremented = false;
    while (iter != noteElement.end() &&
           (iter->getName() == "footnote" || iter->getName() == "level" || iter->getName() == "voice"))
    {
