...
{
Barline::Barline()
    : myAttributes(std::make_shared<BarlineAttributes>()), myBarStyle(makeBarStyle()), myHasBarStyle(false),
      myEditorialGroup(makeEditorialGroup()), myWavyLine(makeWavyLine()), myHasWavyLine(false), mySegno(makeSegno()),
      myHasSegno(false), myCoda(makeCoda()), myHasCoda(false), myFermataSet(), myEnding(makeEnding()),
      myHasEnding(false), myRepeat(makeRepeat()), myHasRepeat(false)
{
}
...
{
    return myHasBarStyle || myEditorialGroup->hasContents() || myHasWavyLine || myHasSegno || myHasCoda ||
           myFermataSet.size() > 0 || myHasEnding || myHasRepeat;
}
...
bool Barline::fromXElementImpl(std::ostream &message, ::ezxml::XElement &xelement)
{
    ...
    {
        if (importElement(message, *it, isSuccess, *myBarStyle, myHasBarStyle))
        {
            continue;
        }
        importGroup(message, it, endIter, isSuccess, myEditorialGroup);
        if (importElement(message, *it, isSuccess, *myWavyLine, myHasWavyLine))
        {
            continue;
        }
        if (importElement(message, *it, isSuccess, *mySegno, myHasSegno))
        {
            continue;
        }
        if (importElement(message, *it, isSuccess, *myCoda, myHasCoda))
        {
            continue;
        }
        importElementSet(message, it, endIter, isSuccess, "fermata", myFermataSet);
        if (importElement(message, *it, isSuccess, *myEnding, myHasEnding))
        {
            continue;
        }
        if (importElement(message, *it, isSuccess, *myRepeat, myHasRepeat))
        {
            continue;
        }
    }
