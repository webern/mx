...
{
Credit::Credit()
    : myAttributes(std::make_shared<CreditAttributes>()), myCreditTypeSet(), myLinkSet(), myBookmarkSet(),
...
#define MX_CREDIT_RETURN_IF_END                                                                                        \
    ...
    {                                                                                                                  \
        ...
        {                                                                                                              \
            message << "Credit: neither 'credit-words' nor 'credit-image' was present - one of these is required"      \
                    << std::endl;                                                                                      \
            isSuccess = false;                                                                                         \
        }                                                                                                              \
        return isSuccess;                                                                                              \
...
bool Credit::fromXElementImpl(std::ostream &message, ::ezxml::XElement &xelement)
{
    ...
    if (i->getName() == "credit-image" || i->getName() == "credit-words")
    {
        ...
        if (i->getName() == "credit-words")
        {
            ...
            while (i != endIter &&
                   ((i->getName() == "link") || (i->getName() == "bookmark") || (i->getName() == "credit-words")))
            {
                while (i != endIter && i->getName() == "link")
                {
                    auto link = makeLink();
