...
inline EncoderPtr makeEncoder()
{
    return std::make_shared<Encoder>();
}

inline EncoderPtr makeEncoder(const XsString &value)
{
    return std::make_shared<Encoder>(value);
}

inline EncoderPtr makeEncoder(XsString &&value)
{
    return std::make_shared<Encoder>(std::move(value));
}
