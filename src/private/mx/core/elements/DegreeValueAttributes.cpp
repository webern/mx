...
bool DegreeValueAttributes::fromXElementImpl(std::ostream &message, ::ezxml::XElement &xelement)
{
    ...
    for (; it != endIter; ++it)
    {
        if (parseAttribute(message, it, className, isSuccess, symbol, hasSymbol, "symbol", &parseDegreeSymbolValue))
        {
            continue;
        }
