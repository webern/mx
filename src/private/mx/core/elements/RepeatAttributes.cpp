...
{
    return hasDirection || hasTimes || hasWinged;
}
...
bool RepeatAttributes::fromXElementImpl(std::ostream &message, ::ezxml::XElement &xelement)
{
    ...
    for (; it != endIter; ++it)
    {
        if (parseAttribute(message, it, className, isSuccess, direction, isDirectionFound, "direction",
                           &parseBackwardForward))
        {
            continue;
        }
        if (parseAttribute(message, it, className, isSuccess, times, hasTimes, "times"))
        {
            continue;
        }
        if (parseAttribute(message, it, className, isSuccess, winged, hasWinged, "winged", &parseWinged))
        {
            continue;
        }
    }
