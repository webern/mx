...
{
    return hasNumber || hasShowFrets || hasPrintObject || hasPrintSpacing;
}
...
bool StaffDetailsAttributes::fromXElementImpl(std::ostream &message, ::ezxml::XElement &xelement)
{
    ...
    for (; it != endIter; ++it)
    {
        ...
        if (parseAttribute(message, it, className, isSuccess, showFrets, hasShowFrets, "show-frets", &parseShowFrets))
        {
            continue;
        }
