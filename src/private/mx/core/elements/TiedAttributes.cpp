...
TiedAttributes::TiedAttributes()
    : type(StartStopContinue::start), number(), lineType(LineType::solid), dashLength(), spaceLength(), defaultX(),
      ...
      hasRelativeX(false), hasRelativeY(false), hasPlacement(false), hasOrientation(false), hasBezierOffset(false),
      hasBezierOffset2(false), hasBezierX(false), hasBezierY(false), hasBezierX2(false), hasBezierY2(false),
      hasColor(false)
{
}
...
bool TiedAttributes::fromXElementImpl(std::ostream &message, ::ezxml::XElement &xelement)
{
    const char *const className = "TiedAttributes";
    bool isSuccess = true;
    ...
    for (; it != endIter; ++it)
    {
        ...
        if (parseAttribute(message, it, className, isSuccess, orientation, hasOrientation, "orientation",
                           &parseOverUnder))
        {
            continue;
        }
        if (parseAttribute(message, it, className, isSuccess, bezierOffset, hasBezierOffset, "bezier-offset"))
        {
            continue;
        }
        ...
        if (parseAttribute(message, it, className, isSuccess, bezierX, hasBezierX, "bezier-x"))
        {
            continue;
        }
        if (parseAttribute(message, it, className, isSuccess, bezierY, hasBezierY, "bezier-y"))
        {
            continue;
        }
