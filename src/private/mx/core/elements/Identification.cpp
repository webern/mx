...
Identification::Identification()
    : myCreatorSet(), myRightsSet(), myEncoding(makeEncoding()), myHasEncoding(false), mySource(makeSource()),
      myHasSource(false), myRelationSet(), myMiscellaneous(makeMiscellaneous()), myHasMiscellaneous(false)
{
}
...
{
    return myCreatorSet.size() > 0 || myRightsSet.size() > 0 || myHasEncoding || myHasSource ||
           myRelationSet.size() > 0 || myHasMiscellaneous;
}
...
bool Identification::fromXElementImpl(std::ostream &message, ::ezxml::XElement &xelement)
{
    ...
    for (auto it = xelement.begin(); it != endIter; ++it)
    {
        ...
        if (importElement(message, *it, isSuccess, *myEncoding, myHasEncoding))
        {
            continue;
        }
        if (importElement(message, *it, isSuccess, *mySource, myHasSource))
        {
            continue;
        }
        importElementSet(message, it, endIter, isSuccess, "relation", myRelationSet);
        if (importElement(message, *it, isSuccess, *myMiscellaneous, myHasMiscellaneous))
        {
            continue;
        }
    }
