...
{
Hole::Hole()
    : myAttributes(std::make_shared<HoleAttributes>()), myHoleType(makeHoleType()), myHasHoleType(false),
      myHoleClosed(makeHoleClosed()), myHoleShape(makeHoleShape()), myHasHoleShape(false)
{
}
...
bool Hole::fromXElementImpl(std::ostream &message, ::ezxml::XElement &xelement)
{
    bool isSuccess = true;
    ...
    {
        if (importElement(message, *it, isSuccess, *myHoleType, myHasHoleType))
        {
            continue;
        }
        ...
        if (importElement(message, *it, isSuccess, *myHoleShape, myHasHoleShape))
        {
            continue;
        }
    }
