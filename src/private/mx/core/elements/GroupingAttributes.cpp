...
GroupingAttributes::GroupingAttributes()
    : type(StartStopSingle::single), number(XsToken("1")), memberOf(), hasType(true), hasNumber(false),
      hasMemberOf(false)
{
}
...
{
    return hasType || hasNumber || hasMemberOf;
}
...
bool GroupingAttributes::fromXElementImpl(std::ostream &message, ::ezxml::XElement &xelement)
{
    ...
    for (; it != endIter; ++it)
    {
        if (parseAttribute(message, it, className, isSuccess, type, isTypeFound, "type", &parseStartStopSingle))
        {
            continue;
        }
        ...
        if (parseAttribute(message, it, className, isSuccess, memberOf, hasMemberOf, "member-of"))
        {
            continue;
        }
    }
