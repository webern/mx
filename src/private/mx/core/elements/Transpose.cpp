...
{
Transpose::Transpose()
    : myAttributes(std::make_shared<TransposeAttributes>()), myDiatonic(makeDiatonic()), myHasDiatonic(false),
      myChromatic(makeChromatic()), myOctaveChange(makeOctaveChange()), myHasOctaveChange(false),
      myDouble(makeDouble()), myHasDouble(false)
{
}
...
bool Transpose::fromXElementImpl(std::ostream &message, ::ezxml::XElement &xelement)
{
    ...
    {
        if (importElement(message, *it, isSuccess, *myDiatonic, myHasDiatonic))
        {
            continue;
        }
        if (importElement(message, *it, isSuccess, *myChromatic, isChromaticFound))
        {
            continue;
        }
        if (importElement(message, *it, isSuccess, *myOctaveChange, myHasOctaveChange))
        {
            continue;
        }
        if (importElement(message, *it, isSuccess, *myDouble, myHasDouble))
        {
            continue;
        }
    }
