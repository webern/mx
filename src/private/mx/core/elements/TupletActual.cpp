...
TupletActual::TupletActual()
    : myTupletNumber(makeTupletNumber()), myHasTupletNumber(false), myTupletType(makeTupletType()),
      myHasTupletType(false), myTupletDotSet()
{
}
...
bool TupletActual::fromXElementImpl(std::ostream &message, ::ezxml::XElement &xelement)
{
    bool isSuccess = true;
    ...
    {
        if (importElement(message, *it, isSuccess, *myTupletNumber, myHasTupletNumber))
        {
            continue;
        }
        if (importElement(message, *it, isSuccess, *myTupletType, myHasTupletType))
        {
            continue;
        }
