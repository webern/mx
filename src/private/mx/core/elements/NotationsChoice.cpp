...
NotationsChoice::NotationsChoice()
    : myChoice(Choice::tied), myTied(makeTied()), mySlur(makeSlur()), myTuplet(makeTuplet()),
      myGlissando(makeGlissando()), mySlide(makeSlide()), myOrnaments(makeOrnaments()), myTechnical(makeTechnical()),
      myArticulations(makeArticulations()), myDynamics(makeDynamics()), myFermata(makeFermata()),
      myArpeggiate(makeArpeggiate()), myNonArpeggiate(makeNonArpeggiate()), myAccidentalMark(makeAccidentalMark()),
