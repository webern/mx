...
StaffDetails::StaffDetails()
    : myAttributes(std::make_shared<StaffDetailsAttributes>()), myStaffType(makeStaffType()), myHasStaffType(false),
      myStaffLines(makeStaffLines(NonNegativeInteger(5))), myHasStaffLines(false), myStaffTuningSet(),
      myCapo(makeCapo()), myHasCapo(false), myStaffSize(makeStaffSize()), myHasStaffSize(false)
{
}
...
{
    return myHasStaffType || myHasStaffLines || myStaffTuningSet.size() > 0 || myHasCapo || myHasStaffSize;
}
...
bool StaffDetails::fromXElementImpl(std::ostream &message, ::ezxml::XElement &xelement)
{
    bool isSuccess = true;
    ...
    {
        if (importElement(message, *it, isSuccess, *myStaffType, myHasStaffType))
        {
            continue;
        }
        if (importElement(message, *it, isSuccess, *myStaffLines, myHasStaffLines))
        {
            continue;
        }
        importElementSet(message, it, endIter, isSuccess, "staff-tuning", myStaffTuningSet);
        if (importElement(message, *it, isSuccess, *myCapo, myHasCapo))
        {
            continue;
        }
        if (importElement(message, *it, isSuccess, *myStaffSize, myHasStaffSize))
        {
            continue;
        }
    }
