...
constexpr const char *const directions[] = {"rehearsal",  "segno",          "words",           "coda",
                                            "wedge",      "dynamics",       "dashes",          "bracket",
                                            "pedal",      "metronome",      "octave-shift",    "harp-pedals",
                                            "damp",       "damp-all",       "eyeglasses",      "string-mute",
                                            "scordatura", "image",          "principal-voice", "accordion-registration",
                                            "percussion", "other-direction"};

constexpr const size_t multiDirectionsSize = 6;
constexpr const char *const multiDirections[] = {"rehearsal", "segno", "words", "coda", "dynamics", "percussion"};
} // namespace
...
{
Direction::Direction()
    : myAttributes(std::make_shared<DirectionAttributes>()), myDirectionTypeSet(), myOffset(makeOffset()),
      myHasOffset(false), myEditorialVoiceDirectionGroup(makeEditorialVoiceDirectionGroup()), myStaff(makeStaff()),
      myHasStaff(false), mySound(makeSound()), myHasSound(false)
{
...
bool Direction::fromXElementImpl(std::ostream &message, ::ezxml::XElement &xelement)
{
    ...
    {
        if (importDirectionTypeSet(message, it, endIter, isSuccess, isDirectionTypeFound))
        {
            continue;
        }
        ...
        if (importElement(message, *it, isSuccess, *myStaff, myHasStaff))
        {
            continue;
        }
        if (importElement(message, *it, isSuccess, *mySound, myHasSound))
        {
            continue;
        }
    }
...
bool Direction::importDirectionTypeSet(std::ostream &message, ::ezxml::XElementIterator &iter,
                                       ::ezxml::XElementIterator &endIter, bool &isSuccess, bool &isFound)
{
    if (iter == endIter)
    {
        isFound = false;
...
DirectionTypePtr Direction::createDirectionType(std::ostream &message, ::ezxml::XElementIterator &subIter,
                                                ::ezxml::XElementIterator &subIterEnd, bool &isSuccess)
{
    ...
    if (subIter->getName() == name)
    {
        ...
        while (subIter != subIterEnd)
        {
            if (subIter->getName() != name)
            {
                message << "Direction: createDirectionType encountered an unexpected element '" << subIter->getName()
                        << "' while parsing a collection of '" << name << "' elements" << std::endl;
                isSuccess = false;
                return directionType;
            }
