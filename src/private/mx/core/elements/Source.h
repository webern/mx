...
inline SourcePtr makeSource(const XsString &value)
{
    return std::make_shared<Source>(value);
}

inline SourcePtr makeSource(XsString &&value)
{
    return std::make_shared<Source>(std::move(value));
}
