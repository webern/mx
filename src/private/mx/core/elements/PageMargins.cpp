...
PageMargins::PageMargins()
    : myAttributes(std::make_shared<PageMarginsAttributes>()), myLeftMargin(makeLeftMargin()),
      myRightMargin(makeRightMargin()), myTopMargin(makeTopMargin()), myBottomMargin(makeBottomMargin())
{
}
...
bool PageMargins::fromXElementImpl(std::ostream &message, ::ezxml::XElement &xelement)
{
    bool isSuccess = true;
    ...
    {
        if (importElement(message, *it, isSuccess, *myLeftMargin, isLeftMarginFound))
        {
            continue;
        }
        if (importElement(message, *it, isSuccess, *myRightMargin, isRightMarginFound))
        {
            continue;
        }
        if (importElement(message, *it, isSuccess, *myTopMargin, isTopMarginFound))
        {
            continue;
        }
        if (importElement(message, *it, isSuccess, *myBottomMargin, isBottomMarginFound))
        {
            continue;
        }
    }
