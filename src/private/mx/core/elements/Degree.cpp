...
bool Degree::fromXElementImpl(std::ostream &message, ::ezxml::XElement &xelement)
{
    bool isSuccess = true;
    ...
    {
        if (importElement(message, *it, isSuccess, *myDegreeValue, isDegreeValueFound))
        {
            continue;
        }
        if (importElement(message, *it, isSuccess, *myDegreeAlter, isDegreeAlterFound))
        {
            continue;
        }
        if (importElement(message, *it, isSuccess, *myDegreeType, isDegreeTypeFound))
        {
            continue;
        }
    }
