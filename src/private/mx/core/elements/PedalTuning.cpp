...
namespace core
{
PedalTuning::PedalTuning() : myPedalStep(makePedalStep()), myPedalAlter(makePedalAlter())
{
}
...
bool PedalTuning::fromXElementImpl(std::ostream &message, ::ezxml::XElement &xelement)
{
    bool isSuccess = true;
    ...
    {
        if (importElement(message, *it, isSuccess, *myPedalStep, isPedalStepFound))
        {
            continue;
        }
        if (importElement(message, *it, isSuccess, *myPedalAlter, isPedalAlterFound))
        {
            continue;
        }
    }
