...
MidiInstrument::MidiInstrument()
    : myAttributes(std::make_shared<MidiInstrumentAttributes>()), myMidiChannel(makeMidiChannel()),
      myHasMidiChannel(false), myMidiName(makeMidiName()), myHasMidiName(false), myMidiBank(makeMidiBank()),
      myHasMidiBank(false), myMidiProgram(makeMidiProgram()), myHasMidiProgram(false),
      myMidiUnpitched(makeMidiUnpitched()), myHasMidiUnpitched(false), myVolume(makeVolume()), myHasVolume(false),
      myPan(makePan()), myHasPan(false), myElevation(makeElevation()), myHasElevation(false)
{
}
...
{
    return myHasMidiChannel || myHasMidiName || myHasMidiBank || myHasMidiProgram || myHasMidiUnpitched ||
           myHasVolume || myHasPan || myHasElevation;
}
...
bool MidiInstrument::fromXElementImpl(std::ostream &message, ::ezxml::XElement &xelement)
{
    ...
    {
        if (importElement(message, *it, isSuccess, *myMidiChannel, myHasMidiChannel))
        {
            continue;
        }
        if (importElement(message, *it, isSuccess, *myMidiName, myHasMidiName))
        {
            continue;
        }
        if (importElement(message, *it, isSuccess, *myMidiBank, myHasMidiBank))
        {
            continue;
        }
        if (importElement(message, *it, isSuccess, *myMidiProgram, myHasMidiProgram))
        {
            continue;
        }
        if (importElement(message, *it, isSuccess, *myMidiUnpitched, myHasMidiUnpitched))
        {
            continue;
        }
        if (importElement(message, *it, isSuccess, *myVolume, myHasVolume))
        {
            continue;
        }
        if (importElement(message, *it, isSuccess, *myPan, myHasPan))
        {
            continue;
        }
        if (importElement(message, *it, isSuccess, *myElevation, myHasElevation))
        {
            continue;
        }
    }
