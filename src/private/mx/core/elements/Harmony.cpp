...
{
Harmony::Harmony()
    : myAttributes(std::make_shared<HarmonyAttributes>()), myHarmonyChordGroupSet(), myFrame(makeFrame()),
      myHasFrame(false), myOffset(makeOffset()), myHasOffset(false), myEditorialGroup(makeEditorialGroup()),
      myStaff(makeStaff()), myHasStaff(false)
{
...
bool Harmony::fromXElementImpl(std::ostream &message, ::ezxml::XElement &xelement)
{
    bool isSuccess = true;
    ...
    for (auto it = xelement.begin(); it != endIter; ++it)
    {
        ...
        if (elementName == "root" || elementName == "function")
        {
            ...
            {
                message << "Harmony: 'kind' is a required element and is missing aborting" << std::endl;
                return false;
            }
        ...
        if (importElement(message, *it, isSuccess, *myFrame, myHasFrame))
        {
            continue;
        }
