...
namespace core
{
SystemDividers::SystemDividers() : myLeftDivider(makeLeftDivider()), myRightDivider(makeRightDivider())
{
}
...
bool SystemDividers::fromXElementImpl(std::ostream &message, ::ezxml::XElement &xelement)
{
    bool isSuccess = true;
    ...
    {
        if (importElement(message, *it, isSuccess, *myLeftDivider, isLeftDividerFound))
        {
            continue;
        }
        if (importElement(message, *it, isSuccess, *myRightDivider, isRightDividerFound))
        {
            continue;
        }
    }
