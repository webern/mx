...
LyricAttributes::LyricAttributes()
    : number(), name(), justify(LeftCenterRight::center), defaultX(), defaultY(), relativeX(), relativeY(),
      placement(AboveBelow::below), color(), printObject(YesNo::no), hasNumber(false), hasName(false),
      hasJustify(false), hasDefaultX(false), hasDefaultY(false), hasRelativeX(false), hasRelativeY(false),
      hasPlacement(false), hasColor(false), hasPrintObject(false)
{
}

bool LyricAttributes::hasValues() const
{
    return hasNumber || hasName || hasJustify || hasDefaultX || hasDefaultY || hasRelativeX || hasRelativeY ||
           hasPlacement || hasColor || hasPrintObject;
}
...
bool LyricAttributes::fromXElementImpl(std::ostream &message, ::ezxml::XElement &xelement)
{
    ...
    for (; it != endIter; ++it)
    {
        ...
        if (parseAttribute(message, it, className, isSuccess, name, hasName, "name"))
        {
            continue;
        }
