...
SystemLayout::SystemLayout()
    : mySystemMargins(makeSystemMargins()), myHasSystemMargins(false), mySystemDistance(makeSystemDistance()),
      myHasSystemDistance(false), myTopSystemDistance(makeTopSystemDistance()), myHasTopSystemDistance(false),
      mySystemDividers(makeSystemDividers()), myHasSystemDividers(false)
{
}
...
{
    return myHasSystemMargins || myHasSystemDistance || myHasTopSystemDistance || myHasSystemDividers;
}
...
bool SystemLayout::fromXElementImpl(std::ostream &message, ::ezxml::XElement &xelement)
{
    bool isSuccess = true;
    ...
    {
        if (importElement(message, *it, isSuccess, *mySystemMargins, myHasSystemMargins))
        {
            continue;
        }
        if (importElement(message, *it, isSuccess, *mySystemDistance, myHasSystemDistance))
        {
            continue;
        }
        if (importElement(message, *it, isSuccess, *myTopSystemDistance, myHasTopSystemDistance))
        {
            continue;
        }
        if (importElement(message, *it, isSuccess, *mySystemDividers, myHasSystemDividers))
        {
            continue;
        }
    }
