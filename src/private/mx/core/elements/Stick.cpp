...
{
Stick::Stick()
    : myAttributes(std::make_shared<StickAttributes>()), myStickType(makeStickType()),
...
bool Stick::fromXElementImpl(std::ostream &message, ::ezxml::XElement &xelement)
{
    ...
    {
        if (importElement(message, *it, isSuccess, *myStickType, isStickTypeFound))
        {
            continue;
        }
        if (importElement(message, *it, isSuccess, *myStickMaterial, isStickMaterialFound))
        {
            continue;
        }
    }
