...
{
Clef::Clef()
    : myAttributes(std::make_shared<ClefAttributes>()), mySign(makeSign()), myLine(makeLine()), myHasLine(false),
      myClefOctaveChange(makeClefOctaveChange()), myHasClefOctaveChange(false)
{
}
...
bool Clef::fromXElementImpl(std::ostream &message, ::ezxml::XElement &xelement)
{
    bool isSuccess = true;
    ...
    {
        if (importElement(message, *it, isSuccess, *mySign, isSignFound))
        {
            continue;
        }
        if (importElement(message, *it, isSuccess, *myLine, myHasLine))
        {
            continue;
        }
        if (importElement(message, *it, isSuccess, *myClefOctaveChange, myHasClefOctaveChange))
        {
            continue;
        }
    }
