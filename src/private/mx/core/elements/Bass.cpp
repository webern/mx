...
namespace core
{
Bass::Bass() : myBassStep(makeBassStep()), myBassAlter(makeBassAlter()), myHasBassAlter(false)
{
}
...
bool Bass::fromXElementImpl(std::ostream &message, ::ezxml::XElement &xelement)
{
    bool isSuccess = true;
    ...
    {
        if (importElement(message, *it, isSuccess, *myBassStep, isBassStepFound))
        {
            continue;
        }
        if (importElement(message, *it, isSuccess, *myBassAlter, myHasBassAlter))
        {
            continue;
        }
    }
