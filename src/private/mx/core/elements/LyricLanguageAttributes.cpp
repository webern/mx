...
LyricLanguageAttributes::LyricLanguageAttributes()
    : number(), name(), lang("it"), hasNumber(false), hasName(false), hasLang(true)
{
}

bool LyricLanguageAttributes::hasValues() const
{
    return hasNumber || hasName || hasLang;
}
...
bool LyricLanguageAttributes::fromXElementImpl(std::ostream &message, ::ezxml::XElement &xelement)
{
    ...
    for (; it != endIter; ++it)
    {
        ...
        if (parseAttribute(message, it, className, isSuccess, lang, isLangFound, "xml:lang"))
        {
            continue;
        }
