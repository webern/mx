...
AccordionRegistration::AccordionRegistration()
    : myAttributes(std::make_shared<AccordionRegistrationAttributes>()), myAccordionHigh(makeAccordionHigh()),
      myHasAccordionHigh(false), myAccordionMiddle(makeAccordionMiddle()), myHasAccordionMiddle(false),
      myAccordionLow(makeAccordionLow()), myHasAccordionLow(false)
{
}
...
bool AccordionRegistration::fromXElementImpl(std::ostream &message, ::ezxml::XElement &xelement)
{
    bool isSuccess = true;
    ...
    {
        if (importElement(message, *it, isSuccess, *myAccordionHigh, myHasAccordionHigh))
        {
            continue;
        }
        if (importElement(message, *it, isSuccess, *myAccordionMiddle, myHasAccordionMiddle))
        {
            continue;
        }
        if (importElement(message, *it, isSuccess, *myAccordionLow, myHasAccordionLow))
        {
            continue;
        }
    }
