...
{
Work::Work()
    : myWorkNumber(makeWorkNumber()), myHasWorkNumber(false), myWorkTitle(makeWorkTitle()), myHasWorkTitle(false),
      myOpus(makeOpus()), myHasOpus(false)
{
}
...
bool Work::fromXElementImpl(std::ostream &message, ::ezxml::XElement &xelement)
{
    ...
    {
        if (importElement(message, *it, isSuccess, *myWorkNumber, myHasWorkNumber))
        {
            continue;
        }
        if (importElement(message, *it, isSuccess, *myWorkTitle, myHasWorkTitle))
        {
            continue;
        }
        if (importElement(message, *it, isSuccess, *myOpus, myHasOpus))
        {
            continue;
        }
    }
