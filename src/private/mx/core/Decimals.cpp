...
PreciseDecimal::PreciseDecimal(short inMaxIntegerDigits, short inMaxDecimalDigits, DecimalType inValue)
    : myIsNegative{inValue < 0.0}, myInteger{0}, myDecimal{0}, myMaxIntegerDigits{DEFAULT_MAX_INTEGER_DIGITS},
...
PreciseDecimal::PreciseDecimal() : PreciseDecimal{DEFAULT_MAX_INTEGER_DIGITS, DEFAULT_MAX_DECIMAL_DIGITS, 0.0}
{
}
...
std::ostream &PreciseDecimal::toStream(std::ostream &os) const
{
    ...
    for (auto c = decIntStr.crbegin(); c != decIntStr.crend(); ++c)
    {
...
DecimalType PreciseDecimal::express(uint64_t inInteger, uint64_t inDecimal, short inMaxDecimalDigits, bool inIsNegative)
{
    ...
    const auto powerMultiplierAsFloat =
        std::pow(static_cast<DecimalType>(10), static_cast<DecimalType>(inMaxDecimalDigits));
...
Decimal::Decimal(DecimalType value) : myValue{19, DEFAULT_PRECISION, value}
{
}
...
DecimalRange::DecimalRange(DecimalClamp min, DecimalClamp max, DecimalType value)
    : Decimal{min(max(value))}, myMinClamp{min}, myMaxClamp{max}
{
}
...
DivisionsValue::DivisionsValue(DecimalType value) : DecimalRange{MX_NOOP, MX_NOOP, value}
{
}

DivisionsValue::DivisionsValue() : DivisionsValue{0.0}
{
}

MillimetersValue::MillimetersValue(DecimalType value) : DecimalRange{MX_NOOP, MX_NOOP, value}
{
}

MillimetersValue::MillimetersValue() : MillimetersValue{0.0}
{
}

NonNegativeDecimal::NonNegativeDecimal(DecimalType value) : DecimalRange{MXMININ(0), MX_NOOP, value}
{
}

NonNegativeDecimal::NonNegativeDecimal() : NonNegativeDecimal{0.0}
{
}

Percent::Percent(DecimalType value) : DecimalRange{MXMININ(0), MXMAXIN(100), value}
{
}
...
PositiveDecimal::PositiveDecimal(DecimalType value) : DecimalRange{MXMINEX(0), MX_NOOP, value}
{
}

PositiveDecimal::PositiveDecimal() : PositiveDecimal{1}
{
}

PositiveDivisions::PositiveDivisions(DecimalType value) : DecimalRange{MXMINEX(0), MX_NOOP, value}
{
}

PositiveDivisions::PositiveDivisions() : PositiveDivisions{1}
{
}

PositiveDivisionsValue::PositiveDivisionsValue(DecimalType value) : DecimalRange{MXMINEX(0), MX_NOOP, value}
{
}

PositiveDivisionsValue::PositiveDivisionsValue() : PositiveDivisionsValue{1}
{
}

RotationDegrees::RotationDegrees(DecimalType value) : DecimalRange{MXMININ(-180), MXMAXIN(180), value}
{
}

RotationDegrees::RotationDegrees() : RotationDegrees{0.0}
{
}

Semitones::Semitones(DecimalType value) : DecimalRange{MX_NOOP, MX_NOOP, value}
{
}

Semitones::Semitones() : Semitones{0.0}
{
}

TenthsValue::TenthsValue(DecimalType value) : DecimalRange{MX_NOOP, MX_NOOP, value}
{
}

TenthsValue::TenthsValue() : TenthsValue{0.0}
{
}

TrillBeats::TrillBeats(DecimalType value) : DecimalRange{MXMININ(2), MX_NOOP, value}
{
}

TrillBeats::TrillBeats() : TrillBeats{2}
{
}
} // namespace core
} // namespace mx
